...
function [output, p, q] = phansalkar(image, varargin)
% Initialization
...
optargs = {1 10 [15 15] 0.25 'replicate'}; % set defaults
 
optargs(1:numvarargs) = varargin;   % use memorable variable names
[p,q, window, k, padding] = optargs{:};
...
% Convert to double
image = double(rescale(image));%./max(double(image),[],'all');

% Mean value
avg = averagefilter(image, window, padding);

% Standard deviation
meanSquare = averagefilter(image.^2, window, padding);
deviation = (meanSquare - avg.^2).^0.5;

% Phansalkar
R = max(max(deviation(:)));
threshold = avg.*(1 + p*exp(-q*avg) + k * ((deviation / R)-1));
